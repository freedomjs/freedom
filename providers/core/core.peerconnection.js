...
function SimpleDataPeer(peerName, dispatchEvent, stunServers,
  ...
  this.pc = new RTCPeerConnectionAdapter.provider(undefined, dispatchEvent, config);//, constraints);
  console.log(this.pc);
  // Add basic event handlers.
  this.pc.onicecandidate = this.onIceCallback.bind(this);
...
SimpleDataPeer.prototype.createOffer = function (constraints, continuation) {
  this.pc.createOffer(constraints).then(continuation).catch(
    function(e) {
      console.error('core.peerconnection createOffer failed:' + e.message);
    });
};
...
SimpleDataPeer.prototype.openDataChannel = function (channelId, continuation) {
  this.pc.createDataChannel(channelId, {}).then(function(id) {
    continuation();
  });
  ...
  SimpleDataPeer.prototype.handleSignalMessage = function (messageText) {
    ...
    if (json.sdp) {
      ...
      this.pc.setRemoteDescription(new this.RTCSessionDescription(json.sdp)).then(
        this.pc.getRemoteDescription).then(
          function(remoteDescription) {
            if (remoteDescription.type === "offer") {
              this.pc.createAnswer().
                then(this.onDescription.bind(this)).
                catch(console.error);
            }
  ...
  PeerConnection.prototype.setup = function (signallingChannelId, peerName,
    ...
    this.peer = new SimpleDataPeer(this.peerName, this.dispatchEvent, stunServers,
                                   dataChannelCallbacks, mocks);
