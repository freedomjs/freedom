...
define('lib/util',[], function() {
  ...
  var lStore = localStorageOverride || localStorage;
  ...
  var util = {
    ...
    grepLocalStorage: function(pattern, iter) {
      var numLocalStorage = lStore.length;
      var keys = [];
      for(var i=0;i<numLocalStorage;i++) {
        var key = lStore.key(i);
        if(pattern.test(key)) {
          keys.push(key);
        }
      }
    ...
    getSettingStore: function(prefix) {
      ...
      return {
        get: function(key) {
          var data = lStore.getItem(makeKey(key));
        ...
        set: function(key, value) {
          ...
          return lStore.setItem(makeKey(key), value);
        },
        remove: function(key) {
          return lStore.removeItem(makeKey(key));
        },
        clear: function() {
...
define('module/root', ['remoteStorage', '../lib/util'], function(remoteStorage, util) {
